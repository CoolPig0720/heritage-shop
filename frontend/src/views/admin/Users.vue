    ...
    <el-card class="table-card">
      ...
      >
        ...
        <el-table-column prop="role" label="角色" width="120" align="center">
          <template #default="{ row }">
            <el-tag :type="getRoleType(row.role)" size="large">{{
...
import {
  UPLOAD_URL,
  getAvatarUrl as getConfigAvatarUrl,
} from "@/config/api.js";
...
const getAvatarUrl = (avatar) => {
  return getConfigAvatarUrl(avatar);
};
...
const handleAvatarSuccess = (response) => {
  if (response.code === 200) {
    const avatarUrl = getConfigAvatarUrl(response.data);
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      if (isAdd.value) {
        ...
        // 如果修改的是当前登录用户，同步更新 store
        if (form.value.id === userStore.userInfo?.id) {
