  ...
  <div class="detail-page">
    <div class="section-container">
      <el-card class="detail-card" shadow="never" v-loading="loading">
        ...
        <template v-if="detail">
          <div class="detail-header">
            ...
            <div class="detail-meta">
              ...
              >
              <el-link
                v-if="detail.heritageProjectId && detail.heritageProjectName"
                type="primary"
                :underline="false"
                @click="goProjectDetail(detail.heritageProjectId)"
              >
                关联项目：{{ detail.heritageProjectName }}
              </el-link>
            </div>
          </div>
          <el-divider />
        ...
        <el-empty v-else-if="!loading" description="故事不存在" />
      </el-card>
    </div>
  </div>
</template>
...
import { getHeritageStoryDetail } from "@/api/info";
...
const loadDetail = async () => {
  ...
  loading.value = true;
  try {
    const res = await getHeritageStoryDetail(id);
    if (res.code === 200) {
...
const goBack = () => {
  const tab = route.query.tab || "story";
