          ...
          <div class="order-footer">
            <div class="order-total">
              ...
              <span class="total-value"
                >¥{{ formatMoney(order.totalAmount) }}</span
              >
            </div>
            <div class="order-actions">
              ...
              <el-button size="small" @click="openDetail(order.id)"
                >查看详情</el-button
              >
            </div>
          </div>
      ...
      <div v-if="payment" class="pay-body">
        <el-descriptions :column="1" border>
          ...
          <el-descriptions-item label="金额"
            >¥{{ formatMoney(payment.amount) }}</el-descriptions-item
          >
          ...
          <el-descriptions-item label="Pay URL">
            <div class="pay-url">
              ...
              <el-button size="small" text @click="copyText(payment.payUrl)"
                >复制</el-button
              >
            </div>
          </el-descriptions-item>
          <el-descriptions-item label="状态">
            <el-tag :type="getPaymentTagType(payment.status)" effect="light">{{
              payment.status
            }}</el-tag>
          </el-descriptions-item>
    ...
    <el-dialog v-model="detailDialogVisible" title="订单详情" width="720px">
      ...
      <div v-else-if="detailOrder" class="detail-body">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="状态">
            <el-tag
              :type="getStatusTagType(detailOrder.status)"
              effect="light"
              >{{ getStatusText(detailOrder.status) }}</el-tag
            >
          </el-descriptions-item>
          <el-descriptions-item label="总金额"
            >¥{{ formatMoney(detailOrder.totalAmount) }}</el-descriptions-item
          >
          ...
          <el-descriptions-item label="支付时间">{{
            formatDateTime(detailOrder.paidTime)
          }}</el-descriptions-item>
        ...
        <div class="detail-items">
          ...
          >
            ...
            >
              <template #error>
                <img
                  class="detail-item-image-fallback"
                  :src="PLACEHOLDER_IMAGE"
                  alt=""
                />
              </template>
