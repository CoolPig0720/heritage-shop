...
### 8.1 数据库设计与 SQL
- [x] 设计地址表 `user_address`（一对多、默认地址、支持多级行政区/海外）
- [x] 创建地址模块 SQL 脚本（`backend/src/main/resources/sql/user_address.sql`）
- [x] 更新数据库设计文档（`TODO/数据库设计.md`）并补充变更记录

### 8.2 后端实现
- [x] 创建 UserAddress 实体类与 Mapper（继承 BaseMapper）
- [x] 创建地址相关 DTO（新增/更新/返回）
- [x] 创建地址 Service（默认地址规则、物理删除、归属校验）
- [x] 创建地址 Controller
  - [x] POST `/api/user/address` - 新增地址（登录用户）
  - [x] PUT `/api/user/address/{id}` - 修改地址（登录用户）
  - [x] DELETE `/api/user/address/{id}` - 删除地址（物理删除，登录用户）
  - [x] GET `/api/user/address/list` - 查询当前用户地址（登录用户）
  - [x] PUT `/api/user/address/{id}/default` - 设为默认地址（登录用户）

### 8.3 约束与校验
- [x] `user_id` 从 JWT 中取，禁止前端传
- [x] 设置默认地址时保证同一用户只有一个默认地址
- [x] 地区使用 regionNamePath（/ 分隔）存储，支持多级行政区/海外
- [x] 删除地址采用物理删除（符合“数据库中不保留已删记录”的预期）
...
| 2026-01-29  | 完成收货地址模块（表结构/接口/个人中心前端对接），支持多级行政区与物理删除 |
